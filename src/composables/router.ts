...
export function useRouterPush(inSetup = true) {
  ...
  function toHome(newTab = false) {
    ...
    routerPush({ name: routeName('Mengoo') }, newTab);
  }
  ...
  function toLogin(loginModule?: UnionKey.LoginModule, redirectUrl?: string) {
    const module: UnionKey.LoginModule = loginModule || 'login';
  ...
  function toLoginRedirect() {
    ...
    if (query?.redirect) {
      console.log('query?.redirect', query?.redirect);
			let redir = query?.redirect == '/404'?'/':query?.redirect


      routerPush(redir as string);
    } else {
      toHome();
    }
  }

  function getCurRouteMeta() {
