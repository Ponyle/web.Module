...
export const useTabStore = defineStore('tab-store', {
  ...
  actions: {
    ...
    setActiveTab(fullPath: string) {
			if(this.tabs.find(item=>item.fullPath == fullPath)) {
    ...
    addTab(route: RouteLocationNormalizedLoaded) {
      ...
      if (isInTabRoutes(this.tabs, tab.fullPath)) {
        return;
      } else if (tab.meta.newTab || tab.meta.newTab == false) {
				return;
			}
    ...
    handleClickTab(fullPath: string) {
      ...
      const { routerPush } = useRouterPush(false);
      const isActive = this.activeTab === fullPath && this.activeTab ==pathname;
      if (!isActive) {
    ...
    iniTabStore(currentRoute: RouteLocationNormalizedLoaded) {
      ...
      if (!isHome) {
        const currentTab = getTabRouteByVueRoute(currentRoute);
				if (currentTab.meta.newTab || currentTab.meta.newTab != false) {
      ...
      this.tabs = tabs;
			...
			if(currentRoute.meta.newTab || currentRoute.meta.newTab == false) {
				let fullPath = currentRoute.fullPath
				let fullPatheIndex = fullPath.lastIndexOf('/')
				this.setActiveTab(fullPath.substring(0,fullPatheIndex));
			} else {
