...
<path d="M16.3175 3.7688C15.8801 3.27625 15.2859 3 14.6674 3H5.3326C4.71411 3 4.11991 3.27625 3.68255 3.7688C3.24519 4.26135 3 4.92852 3 5.62435V14.3757C3 15.0715 3.24519 15.7386 3.68255 16.2312C4.11991 16.7238 4.71411 17 5.3326 17H14.6674C15.2859 17 15.8801 16.7238 16.3175 16.2312C16.7548 15.7386 17 15.0715 17 14.3757V5.62695C17 4.93112 16.7548 4.26135 16.3175 3.7688ZM7.6674 15.2539H5.3326C5.12717 15.2539 4.92837 15.1627 4.78258 14.9985C4.63679 14.8343 4.55506 14.6128 4.55506 14.3809V5.62695C4.55506 5.39501 4.63679 5.17349 4.78258 5.00931C4.92837 4.84512 5.12717 4.75391 5.3326 4.75391H7.66519V15.2539H7.6674ZM15.4449 14.3783C15.4449 14.6102 15.3632 14.8317 15.2174 14.9959C15.0716 15.1601 14.8728 15.2513 14.6674 15.2513H9.22247V4.7487H14.6674C14.8728 4.7487 15.0716 4.83991 15.2174 5.0041C15.3632 5.16828 15.4449 5.3898 15.4449 5.62174V14.3783Z" fill="#4E5969"/>
      ...
      <!-- User Info (Bottom) -->
      <div class="p-4 border-2 border-gray-100"  v-if="!collapsed && isUserInfoCompleted === 1">
         <n-dropdown :options="userOptions" @select="handleSelect" trigger="hover" placement="top-start">
             <div class="flex items-center gap-3 cursor-pointer" :class="collapsed ? 'justify-center' : ''">
                 ...
                 <div v-if="!collapsed" class="flex flex-col overflow-hidden">
                     <span class="text-base font-medium text-gray-700 truncate">{{ authStore.userInfo?.realname || '用户' }}</span>
                 </div>
             </div>
         </n-dropdown>
      </div>
   ...
   <div class="flex-1 flex flex-col h-full relative">
        ...
        <div class="flex-1 overflow-y-auto p-6" ref="scrollRef" @scroll="handleScroll">
            ...
            <!-- User Info Completion Flow -->
            <div v-if="isUserInfoCompleted === 0" class="flex flex-col h-full">
                <!-- Profile Header -->
                <div class="h-16 flex items-center justify-between px-4 border-b border-gray-100 ">
                    <img src="@/assets/images/tlogo.svg" alt="Logo" class=" object-contain" @click="tohome" style="cursor: pointer;" />

                    <n-dropdown :options="userOptions" @select="handleSelect" trigger="hover" placement="bottom-end">
                ...
                <!-- Step 1: Guide Card -->
                <div v-if="!showProfileForm" class="flex flex-col items-center justify-center h-full">
                     <div class="transform -translate-y-16">
                        <!-- Mascot -->
                        <div class="absolute -top-20 left-1/2 -translate-x-1/2 z-10">
                             <img src="@/assets/images/robot.png" class="w-32 h-32 object-contain drop-shadow-xl" />
                        </div>
                        ...
                        <div class="relative w-full max-w-2xl mx-auto">
                            ...
                            <div class="flex justify-center">
                                <img src="@/assets/images/rbg1.png" class="max-w-full h-auto object-contain drop-shadow-md" />
                            </div>
                            ...
                            <div class="absolute bottom-[10%] left-0 right-0 flex justify-center">
                                <n-button
                                    type="primary"
                                    size="large"
                                    class="!bg-[#E6F0FF] !text-[#2B6DE5] !border-none !rounded-xl !px-12 !py-6 !text-lg !font-bold shadow-sm hover:!bg-[#dbe9ff] hover:shadow-md transition-all"
                                    @click="goToProfileForm"
                                >
                                    前往填写 <n-icon class="ml-2"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg></n-icon>
                                </n-button>
                            </div>
                        </div>
                     </div>
                ...
                <div v-else class="max-w-4xl mx-auto py-8 px-4">
                     <div class="bg-white rounded-2xl shadow-sm p-8 border border-gray-100">
                        <div class="flex items-center mb-8 pb-4 border-b border-gray-100">
                            <n-button quaternary circle @click="handleProfileBack" class="mr-4 hover:bg-gray-100">
                                <template #icon>
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-6 h-6 text-gray-600"><path fill="currentColor" d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/></svg>
                                </template>
                            </n-button>
                            <h2 class="text-2xl font-bold text-gray-800">用户信息完善</h2>
                        </div>

                        <n-form ref="formRef" :model="profileForm" :rules="profileFormRules" label-placement="left" label-width="120" require-mark-placement="right-hanging" size="large">
                            <n-grid :cols="1" :x-gap="24">
                                <n-form-item-gi label="姓名" path="name" required>
                                    <n-input v-model:value="profileForm.name" placeholder="请填写您的姓名(或昵称)" />
                                </n-form-item-gi>
                                <n-form-item-gi label="手机" path="phone" required>
                                    <n-input v-model:value="profileForm.phone" placeholder="请填写您的手机号码" />
                                </n-form-item-gi>
                                <n-form-item-gi label="邮箱" path="email" required>
                                    <n-input v-model:value="profileForm.email" placeholder="请填写您的电子邮箱" />
                                </n-form-item-gi>
																    <n-form-item-gi label="所属院校" path="unitLevel" required>
                                     <n-input v-model:value="profileForm.unitLevel" placeholder="例:南京医科大学基础医学院" />
                                </n-form-item-gi>
                                <n-form-item-gi label="所属单位" path="unit" required>
                                     <n-input v-model:value="profileForm.unit" placeholder="本科/高职高专院校(或医院等级)" />
                                </n-form-item-gi>

                                <n-form-item-gi label="所在地区" path="location" required>
                                     <n-input v-model:value="profileForm.location" placeholder="例:浙江省杭州市" />
                                </n-form-item-gi>
                                <n-form-item-gi label="主要教学职务" path="job" required>
                                     <n-input v-model:value="profileForm.job" placeholder="例:诊断学教研室主任" />
                                </n-form-item-gi>
                                <n-form-item-gi label="学科专业方向" path="major" required>
                                     <n-input v-model:value="profileForm.major" placeholder="例:临床医学心血管内科" />
                                </n-form-item-gi>

                                <n-form-item-gi label="教学层级" path="students" required>
                                     <n-checkbox-group v-model:value="profileForm.students">
                                         <n-space>
                                             <n-checkbox v-for="opt in studentOptions" :key="opt.value" :value="opt.value" :label="opt.label" />
                                         </n-space>
                                     </n-checkbox-group>
                                </n-form-item-gi>
                                <n-form-item-gi label="常授课程模块" path="course" required>
                                     <n-input v-model:value="profileForm.course" placeholder="例:临床医学心血管内科" />
                                </n-form-item-gi>
                                <n-form-item-gi label="核心教学理念" path="philosophy" required>
                                     <n-input v-model:value="profileForm.philosophy" type="textarea" placeholder="例:我注重在病例中设置递进式悬念，引导学生自主探究诊断" :autosize="{ minRows: 3, maxRows: 5 }" />
                                </n-form-item-gi>
                            </n-grid>

                            <div class="flex justify-center gap-6 mt-10 pb-8">
                                <n-button size="large" class="px-8" @click="handleProfileBack">返回</n-button>
                                <n-button type="primary" size="large" class="px-8" @click="handleProfileSubmit">保存</n-button>
                            </div>
                        </n-form>
                     </div>
                </div>
            ...
            <div v-else class="max-w-4xl mx-auto w-full py-4">
                ...
                <div v-else-if="chatMessages.length === 0" class="flex flex-col items-center justify-center mt-20">
                    <div class="mb-8 relative">
                                              <img src="@/assets/images/robot.png"  /> <!-- Placeholder for Hi mascot -->


                    </div>

                    <h2 class="text-2xl font-bold text-gray-800 mb-8 tracking-tight">有什么我能帮你的吗？</h2>

                    <div class="w-full max-w-4xl px-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div v-for="(suggestion, idx) in [
                                '生成一份关于COPD的 PBL 教案',
                                '我需要一套儿科常见病的数学 PPT 大纲',
                                '帮我拓展一份外科手术的实训教案',
                                '我要设计一堂糖尿病的病例讨论课',
                                '帮我生成一份传染病学的线上微课脚本',
                                '我要优化一份药理学的课堂互动方案'
                            ]" :key="idx"
                            class="bg-white hover:bg-gray-50 border border-gray-100  p-2 cursor-pointer transition-all duration-200 hover:shadow-md hover:border-blue-100 group flex items-center justify-center text-center"
                            @click="quickAsk(suggestion)">
                                <span class="text-gray-600 font-medium text-small group-hover:text-blue-600 transition-colors">{{ suggestion }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="flex flex-col max-w-[100%]" :class="msg.isUser ? 'items-end' : 'items-start'">
                             ...
                             <div v-if="msg.attachment"
                                 ...
                                 <div class="flex-1 min-w-0">
                                   <div class="font-medium text-gray-900 truncate text-base" :title="msg.attachment.fileName">{{ msg.attachment.fileName }}</div>
                             ...
                             <div v-if="!msg.hasMarkdown || !msg.isLoading" class="px-4 py-3 rounded-2xl text-base leading-relaxed"
                                  :class="[
        ...
        <div  class="p-6 backdrop-blur border-t border-gray-100" v-if="isUserInfoCompleted === 1">
          ...
          <div v-if="showLessonPlan" class="w-[40%] border-l border-gray-200 bg-white flex flex-col h-full overflow-hidden">
            ...
            <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-white">
                ...
                <div class="flex items-center gap-2">
                    <!-- Download Button -->
                    <n-button  type="primary" ghost class="!px-3 !py-1 text-sm" @click="exportToWord">
                        <template #icon>
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-4">
                        <div v-for="(section, idx) in lessonPlanData.sections" :key="idx" class="mb-4">
                            <h3 class="font-bold text-gray-900 mb-2 text-base">{{ section.title }}</h3>
                    ...
                    <div v-if="!lessonPlanData.sections || lessonPlanData.sections.length === 0" class="flex flex-col items-center justify-center py-12 text-gray-400">
                        ...
                        <p class="text-base font-medium">教案内容加载中...</p>
                    </div>
                </div>
            </div>
          </div>
...
import { UploadFileByAI, GetLessHistory, GetChatHistory, type ChatRequest, StartLessonionPlan, type StartLessonPlanRequest, GetLessonPlanName, type GetLessonPlanNameRequest,DelLessInfo,JudgeUsers,SaveLessUserInfo } from '@/service';
...
const formRef = ref("")
// User Profile Logic
const isUserInfoCompleted = ref(0); // 0: incomplete, 1: complete
...
const profileFormRules = {
    name: [
        { required: true, message: '请填写您的姓名(或昵称)', trigger: ['blur', 'input'] }
    ],
    phone: [
        { required: true, message: '请填写您的手机号码', trigger: ['blur', 'input'] },
        { pattern: /^1[3-9]\d{9}$/, message: '请输入正确的手机号码', trigger: ['blur', 'input'] }
    ],
    email: [
        { required: true, message: '请填写您的电子邮箱', trigger: ['blur', 'input'] },
        { type: 'email', message: '请输入正确的邮箱格式', trigger: ['blur', 'input'] }
    ],
    unit: [
        { required: true, message: '请填写所属单位', trigger: ['blur', 'input'] }
    ],
    unitLevel: [
        { required: true, message: '请填写所属院校', trigger: ['blur', 'input'] }
    ],
    location: [
        { required: true, message: '请填写所在地区', trigger: ['blur', 'input'] }
    ],
    job: [
        { required: true, message: '请填写主要教学职务', trigger: ['blur', 'input'] }
    ],
    major: [
        { required: true, message: '请填写学科专业方向', trigger: ['blur', 'input'] }
    ],
    teachingLevel: [
        { required: true, message: '请填写主要教学层级', trigger: ['blur', 'input'] }
    ],
    students: [
        { type: 'array', required: true, message: '请至少选择一项', trigger: 'change' }
    ],
    course: [
        { required: true, message: '请填写常授课程模块', trigger: ['blur', 'input'] }
    ],
    philosophy: [
        { required: true, message: '请填写核心教学理念', trigger: ['blur', 'input'] }
    ]
};
        ...
        if (!errors) {
            try {
                ...
                const saveData = {
                    ...
                    affiliatedUnit: profileForm.value.unit,
                    affiliatedSchool: profileForm.value.unitLevel,
                    location: profileForm.value.location,
                    teachPosition: profileForm.value.job,
                    professDirection: profileForm.value.major,
                    teachLevel: profileForm.value.students.join(','),
                    courseModule: profileForm.value.course,
                    teachPhilosophy: profileForm.value.philosophy
                };
								console.log(saveData,'saveData')

                // 保存用户信息
                const saveRes = await SaveLessUserInfo(saveData);

                if (!saveRes.error && saveRes.data) {
                    message.success('保存成功');

                    // 保存成功后，调用inithome判断信息是否完善并更新界面
                    await inithome();
                } else {
...
async function exportToWord() {
	console.log(lessonPlanData.value,'lessonPlanData.value')
  if (!lessonPlanData.value) {
    message.warning('没有可导出的教案内容');
    return;
  }
  ...
  if (lessonPlanData.value.sections && Array.isArray(lessonPlanData.value.sections)) {
    for (const section of lessonPlanData.value.sections) {
      bodyContent += `<h2>${section.title}</h2>`;
      // Convert Markdown to HTML
      if (section.content) {
         try {
           let html = await marked.parse(section.content);

           // Convert lists to flat divs to avoid Word's automatic numbering and formatting issues
           ...
           ols.forEach(ol => {
             ...
             items.forEach((li, index) => {
               ...
               // Strip existing p tags to ensure inline display
               let content = li.innerHTML.replace(/<p[^>]*>/g, '').replace(/<\/p>/g, '');
           ...
           bodyContent += `<div>${html}</div>`;
         } catch (e) {
           console.error('Markdown parse error:', e);
           // Fallback if markdown parsing fails
           bodyContent += `<p>${section.content.replace(/\n/g, '<br>')}</p>`;
         }
      }
    }
  }
  ...
  const fullHtml = `
    ...
    <head>
      ...
      <style>
        ...
        /* Fix list item breaking in Word */
...
const startNewChat = () => {
    ...
    showScrollBottomBtn.value = false; // 重置滚动按钮状态
...
const handleScroll = () => {
  ...
  // 如果没有滚动条（scrollHeight <= clientHeight），不显示按钮
  const isAtBottom = scrollHeight - scrollTop - clientHeight < 100;
  const hasScrollbar = scrollHeight > clientHeight;
  showScrollBottomBtn.value = !isAtBottom && chatMessages.value.length > 0 && hasScrollbar;
};
...
async function inithome() {
  try {
    const res = await JudgeUsers({});
    if (res.data) {
      // 用户已完善信息，显示对话界面
      isUserInfoCompleted.value = 1;
      showProfileForm.value = false;
      showGuide.value = false;
				initializeLessonPlan()
			    inithistoryList();
...
function handleCardClick(msg: any) {
...
function extractMarkdownContent(text: string) {
  ...
  // 支持 ``` 和 markdown 之间有空格
  const completeRegex = /```\s*(?:[Mm]arkdown)?\s*\n?([\s\S]*?)\n?```/;
  ...
  // 支持 ``` 和 markdown 之间有空格
  const partialRegex = /```\s*(?:[Mm]arkdown)?\s*\n?([\s\S]*)$/;
...
async function animateLessonPlanSections(msgIndex: number) {
  ...
  // 内容生成完成后，触发handleCardClick事件以解析title
  handleCardClick(msg);
}
...
async function askAI() {
  if (isUserInfoCompleted.value === 0) {
      message.warning('请先完善用户信息');
      return;
  }
  showGuide.value = false;
        ...
        try {
          ...
          if (!obj.isclose) {
            if (!questionLoading.value) {
              ...
              // 检测是否包含 markdown 开头
              if (obj.content.includes('```') || obj.content.includes('Mark') || obj.content.includes('down')) {
              ...
              // 检测是否包含 markdown 开头
              if (!lastMsg.hasMarkdown && (obj.content.includes('```') || lastMsg.fullContent.includes('```Mark') || lastMsg.fullContent.includes('```down'))) {
