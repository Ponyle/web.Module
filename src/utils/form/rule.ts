...
import { REGEXP_CODE_SIX, REGEXP_EMAIL, REGEXP_PWD2 } from '@/config';
import { t } from '@/locales';
/** 创建自定义错误信息的必填表单规则 */
export const createRequiredFormRule = (message = '不能为空'): FormItemRule => ({ required: true, message, trigger: 'blur' });
...
export const formRules: CustomFormRules = {
  userName: [
    createRequiredFormRule(t('page.login.userNamePlaceholder')),
    // { message: '', trigger: 'blur' }
  ],
  pwd: [
    createRequiredFormRule(t('page.login.passwordPlaceholder')),
    { pattern: REGEXP_PWD2, message: t('page.login.passwordRules'), trigger: ['target', 'blur'] }
  ],
  code: [
