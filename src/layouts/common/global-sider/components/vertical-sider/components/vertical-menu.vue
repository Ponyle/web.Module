...
const menus = computed(() => {
  return routeStore.menus.filter(item => item.icon) as App.GlobalMenuOption[];
});

const activeKey = computed(() => {
  if (route.meta.newTab || route.meta.newTab == false) {
    const name = (route.meta?.activeMenu ? route.meta.activeMenu : route.name) as string;
    const nameIndex = name.lastIndexOf('_');
    return name.substring(0, nameIndex);
  }
  return (route.meta?.activeMenu ? route.meta.activeMenu : route.name) as string;
});
